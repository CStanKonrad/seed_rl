...
def apply_net(action_specs, policy_logits, core_output):
  core_output, zoomed_view = core_output
  n_actions = len(action_specs)
  arr = [policy_logits[i](tf.concat([core_output, zoomed_view[i]]))
...
_STACKS_SPEC = [(16, 2), (32, 2), (32, 2), (32, 2)]
_ZOOM_STACKS_SPEC = [(16, 2), (32, 2), (32, 2), (32, 2)]
_STACKS_TO_LINEAR = 256
_ZOOM_STACKS_TO_LINEAR = 64
...
class GFootball(BaseVTraceNetwork):
  ...
  def __init__(self, action_specs):
    ...
    self._stacks = [
      ...
      for num_ch, num_blocks in _STACKS_SPEC
    ]
    self._zoom_stacks = [
      _Stack(num_ch, num_blocks)
      for num_ch, num_blocks in _ZOOM_STACKS_SPEC
    ]
    self._zoom_to_linear = tf.keras.layers.Dense(
        _ZOOM_STACKS_TO_LINEAR, kernel_initializer='lecun_normal')

    self._conv_to_linear = tf.keras.layers.Dense(
        _STACKS_TO_LINEAR, kernel_initializer='lecun_normal')
  ...
  def _torso(self, unused_prev_action, env_output):
    ...
    zoomed_out = []
    for out in zoomed:
        out = observation.unpackbits(out)
        out /= 255
        for stack in self._zoom_stacks:
            out = stack(out)

        out = tf.nn.relu(out)
        out = tf.keras.layers.Flatten()(out)

        out = self._zoom_to_linear(out)
        zoomed_out.append(tf.nn.relu(out))

    return tf.nn.relu(conv_out), zoomed_out
